...
void fskaitymas (std::vector<duom>& V, int* nd, int tempn)
{
	...
	std::cout << "Failo is " + std::to_string(tempn) + " irasu nuskaitymo laikas: " << diff.count() << " s\n";
}
...
void fisvedimas(vector<duom> V, int nd, int tempn)
{
    ...
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff = end - start;
    std::cout << std::to_string(tempn) + " irasu rusiavimas i dvi grupes uztruko: " << diff.count() << " s\n";

    w1.close();
    w2.close();
    auto end1 = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff1 = end1 - start1;
    std::cout << std::to_string(tempn) + " irasu isvedimas i naujus failus uztruko: " << diff1.count() << " s\n";
}
...
void failogen(vector<duom>& V, int tempn)
{
    auto start = std::chrono::high_resolution_clock::now();
    int randsk;
    randsk = rand()% 3 + 3;
    ...
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff = end - start;
	std::cout << std::to_string(tempn) +" irasu kurimas uztruko: " << diff.count() << " s\n";
}
